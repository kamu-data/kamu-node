...
|---|---|---|---|
| `auth` | [`AuthConfig`](#authconfig) |  | Authentication & authorization |
| `database` | [`DatabaseConfig`](#databaseconfig) |  | Database |
| `datasetEnvVars` | [`DatasetEnvVarsConfig`](#datasetenvvarsconfig) |  | Dataset environment variable feature |
...
| `didEncryption` | [`DidSecretEncryptionConfig`](#didsecretencryptionconfig) |  |  |
| `jwtSecret` | `string` |  |  |
| `passwordPolicy` | [`PasswordPolicyConfig`](#passwordpolicyconfig) |  |  |
| `providers` | `array` |  |  |
...
| Variants |
|---|
| [`InMemory`](#databaseconfiginmemory) |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `gateway` | [`EmailConfigGateway`](#emailconfiggateway) | `V` |  |
| `senderAddress` | `string` | `V` |  |
| `senderName` | `string` |  |  |
...
| Variants |
|---|
| [`Dummy`](#emailconfiggatewaydummy) |
| [`Postmark`](#emailconfiggatewaypostmark) |
...
| `anonSessionInactivityTimeout` | [`DurationString`](#durationstring) |  | Time after which FlightSQL session context will be released to free the<br>resources (for anonymous clients) |
...
| `authedSessionInactivityTimeout` | [`DurationString`](#durationstring) |  | Time after which FlightSQL session context will be released to free the<br>resources (for authenticated clients) |
...
| `preResolveDnslink` | `boolean` |  | Whether kamu should pre-resolve IPNS DNSLink names using DNS or leave it<br>to the Gateway. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `brokerIdleTimeoutMs` | `integer` |  | Time in milliseconds to wait for MQTT broker to send us some data after<br>which we will consider that we have "caught up" and end the polling<br>loop. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `metadataLocalFsCachePath` | `string` |  |  |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `caching` | [`RepoCachingConfig`](#repocachingconfig) |  |  |
| `dataBlocksPageSize` | `integer` |  |  |
| `repoUrl` | [`UrlOrPath`](#urlorpath) |  |  |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `maxBlockingThreads` | `integer` |  |  |
| `threadStackSize` | `integer` |  |  |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `taskCheckingIntervalSecs` | `integer` |  |  |
...
| `storage` | [`UploadRepoStorageConfig`](#uploadrepostorageconfig) |  |  |
...
| [`Local`](#uploadrepostorageconfiglocal) |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `baseUrlFlightsql` | [`UrlOrPath`](#urlorpath) |  |  |
| `baseUrlPlatform` | [`UrlOrPath`](#urlorpath) |  |  |
| `baseUrlRest` | [`UrlOrPath`](#urlorpath) |  |  |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `deliveryTimeoutSecs` | `integer` |  |  |
| `maxConsecutiveFailures` | `integer` |  |  |
| `secretEncryptionEnabled` | `boolean` |  |  |
| `secretEncryptionKey` | `string` |  | Represents the encryption key for the webhooks secret. This field is<br>required if `secret_encryption_enabled` is `true` or `None`.<br><br>The encryption key must be a 32-character alphanumeric string, which<br>includes both uppercase and lowercase Latin letters (A-Z, a-z) and<br>digits (0-9).<br><br># Example<br>let config = WebhooksConfig {<br>    ...<br>    secret_encryption_enabled: Some(true),<br>    encryption_key:<br>Some(String::from("aBcDeFgHiJkLmNoPqRsTuVwXyZ012345")) };  |
