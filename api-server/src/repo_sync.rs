...
use kamu::{
    domain::{MetadataRepository, SyncOptions, SyncResult, SyncService},
...
pub fn repo_sync_loop(catalog: dill::Catalog, repo_id: &RepositoryID) {
    ...
    loop {
        {
            let span = tracing::span!(tracing::Level::INFO, "Synchronizing repository");
...
pub fn sync_all_from_repo(
    ...
    tracing::info!("Syncing {} datasets", datasets_to_sync.len());
    ...
    for ((id, _), res) in sync_results {
        match res {
            ...
            Err(e) => panic!("Failed to sync dataset {}: {}", id, e),
        }
    }
